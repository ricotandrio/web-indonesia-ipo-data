      ...
      <div className="m-14">
        ...
        <div className="flex w-full flex-col mt-5">
          <Tabs aria-label="Options">
              ...
              <Card shadow="sm">
                <CardBody>
                  ...
                  <p>
                    ...
                    {NumberUtil.formatToRupiah(stockInformation?.final_price)}
                  </p>
                  <p>
                    <strong>Listing Board:</strong>{" "}
                    {stockInformation?.listing_board || "N/A"}
                  </p>
                  <p>
                    <strong>Line of Business:</strong>{" "}
                    {stockInformation?.line_of_business}
                  </p>
                  <p>
                  ...
                  <br />
                  <p>
                    <strong>Number of Shares Offered:</strong>{" "}
                    {stockInformation?.number_of_shares_offered} saham
                  </p>
                  <p>
                    <strong>Percentage of Total Shares:</strong>{" "}
                    {stockInformation?.percentage_of_total_shares * 100}%
                  </p>
                  <p>
                    <strong>Participant Admin:</strong>{" "}
                    {stockInformation?.participant_admin?.name} (
                    {stockInformation?.participant_admin?.code})
                  </p>
                </CardBody>
              </Card>
            </Tab>
                ...
                <CardBody>
                  <p>
                    ...
                    {DateUtil.formatToLongDate(
                      stockInformation?.book_building_opening,
                    )}
                  </p>
                  <p>
                    <strong>Closing:</strong>{" "}
                    {DateUtil.formatToLongDate(
                      stockInformation?.book_building_closing,
                    )}
                  </p>
                  <p>
                    <strong>Lowest Price:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.lowest_book_building_price,
                    )}
                  </p>
                  <p>
                    <strong>Highest Price:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.highest_book_building_price,
                    )}
                  </p>
                </CardBody>
              </Card>
            ...
            <Tab
              key="offering-to-listing-period"
              title="Offering to Listing Period"
            >
              <Card shadow="sm">
                <CardBody>
                  <p>
                    <strong>Opening:</strong>{" "}
                    {DateUtil.formatToLongDate(
                      stockInformation?.opening_of_offering_period,
                    )}
                  </p>
                  <p>
                    <strong>Closing:</strong>{" "}
                    {DateUtil.formatToLongDate(
                      stockInformation?.closing_of_offering_period,
                    )}
                  </p>
                  <p>
                    <strong>Closing Date:</strong>{" "}
                    {DateUtil.formatToLongDate(stockInformation?.closing_date)}
                  </p>
                  <p>
                    <strong>Distribution Date:</strong>{" "}
                    {DateUtil.formatToLongDate(
                      stockInformation?.distribution_date,
                    )}
                  </p>
                  <p>
                    <strong>Listing Date:</strong>{" "}
                    {DateUtil.formatToLongDate(stockInformation?.listing_date)}
                  </p>
                </CardBody>
              </Card>
            </Tab>
                  ...
                  <div className="gap-2 sm:gap-3 flex flex-wrap flex-row">
                    {stockInformation?.underwriters?.map((uw, index) => (
                      <Button
                        ...
                        color="primary"
                        variant="flat"
                        onPress={() => navigate(`/uw/${uw?.code}`)}
                      >
                        {uw?.code} - {uw?.name} (
                        {uw?.percentage ? uw?.percentage * 100 : "NaN"}%)
                      </Button>
                    ))}
                  </div>
                </CardBody>
              ...
              <Card shadow="sm">
                <CardBody>
                  ...
                  <p>
                    <strong>Open:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.listing_price?.open,
                    )}
                  </p>
                  <p>
                    <strong>High:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.listing_price?.high,
                    )}
                  </p>
                  <p>
                    <strong>Low:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.listing_price?.low,
                    )}
                  </p>
                  <p>
                    <strong>Close:</strong>{" "}
                    {NumberUtil.formatToRupiah(
                      stockInformation?.listing_price?.close,
                    )}
                  </p>
                  <p>
                    <strong>Change: </strong>
                    <span
                      className={`${stockInformation?.listing_price?.change > 0 ? "text-green-500" : "text-red-500"}`}
                    >
                      {NumberUtil.formatToRupiah(
                        stockInformation?.listing_price?.change,
                      )}{" "}
                      ({stockInformation?.listing_price?.percentage_change})
                    </span>
                  </p>
                  <p>
                    <strong>Volume:</strong>{" "}
                    {stockInformation?.listing_price?.volume}
                  </p>
                </CardBody>
              </Card>
            </Tab>
            <Tab key="performance" title="Performance">
              <Card shadow="sm">
                <CardBody>
                  ...
                  <p>
                    <strong>1D: </strong>
                    {stockInformation?.performa?.["1D"]}
                  </p>
                  <p>
                    <strong>1W: </strong>
                    {stockInformation?.performa?.["1W"]}
                  </p>
                  <p>
                    <strong>1M: </strong>
                    {stockInformation?.performa?.["1M"]}
                  </p>
                  <p>
                    <strong>6M: </strong>
                    {stockInformation?.performa?.["6M"]}
                  </p>
                  <p>
                    <strong>1Y: </strong>
                    {stockInformation?.performa?.["1Y"]}
                  </p>
                  <p>
                    <strong>Updated at: </strong>
                    {stockInformation?.performa?.updated_at}
                  </p>
                  <p>
                    ...
                    <a
                      ...
                      href={stockInformation?.performa?.source}
                    >
                      {stockInformation?.performa?.source}
                    </a>
                  </p>
                </CardBody>
              </Card>
            ...
            <Tab key="fundamental" title="Fundamental">
              <Card shadow="sm">
                <CardBody>
                  ...
                  <p>
                    <strong>Aset: </strong>
                    {stockInformation?.fundamental?.aset_percentage}
                  </p>
                  <p>
                    <strong>Laba Bersih: </strong>
                    {stockInformation?.fundamental?.laba_bersih_percentage}
                  </p>
                  <p>
                    <strong>Liabilitas: </strong>
                    {stockInformation?.fundamental?.liabilitas_percentage}
                  </p>
                  <p>
                    <strong>Pendapatan: </strong>
                    {stockInformation?.fundamental?.pendapatan_percentage}
                  </p>
                  <p>
                    <strong>Updated at: </strong>
                    {stockInformation?.fundamental?.updated_at}
                  </p>
                  <p>
                    <strong>Source: </strong>{" "}
                    <a
                      target="_blank"
                      className="underline text-blue-500"
                      href={stockInformation?.fundamental?.source}
                    >
                      {stockInformation?.fundamental?.source}
                    </a>
                  </p>
                </CardBody>
              </Card>
        ...
        <div className="flex flex-row flex-wrap gap-3 mt-5">
          <Button
            ...
            onPress={() =>
              window.open(
                `https://www.tradingview.com/symbols/IDX-${stockInformation?.ticker_code}`,
                "_blank",
              )
            }
          ...
          <Button
            ...
            onPress={() =>
              window.open(
                `https://e-ipo.co.id/id/ipo/index?query=${stockInformation?.ticker_code}`,
                "_blank",
              )
            }
